...
class EmailService:
    ...
    def _send_smtp_email(self, msg: MIMEMultipart) -> bool:
        """
        Internal synchronous helper to send email via SMTP.
        """
        if not self.enabled:
            logger.info("Email alerts disabled. Skipping email to %s", msg["To"])
            return False
        ...
        try:
            ...
            logger.info("Sent email to %s (subject: %s)", msg["To"], msg["Subject"])
            return True
