...
class ValidationResult:
    """Result from Terraform Registry MCP validation."""
    ok: bool = True
...
class MCPManager:
    ...
    async def gather_registry_context(self, prompt: str, session_id: str = None) -> str:
        ...
        try:
            # Support multi-word commands like "docker run -i --rm hashicorp/terraform-mcp-server"
            parts = shlex.split(config.TERRAFORM_MCP_SERVER)
            server_params = StdioServerParameters(
                command=parts[0],
                args=parts[1:],
                env=None,
                ...
                async with ClientSession(read, write) as session:
                    ...
                    for _ in range(8):
                        ...
                        try:
                            ...
                            logger.error("MCP Registry Tool '%s' failed: %s", fc.name, e)
    ...
    async def validate_terraform(self, main_tf: str) -> ValidationResult:
        ...
        # Extract aws_* resource types from generated HCL
        resource_types_found = re.findall(r'resource\s+"(aws_[\w]+)"', main_tf) if main_tf else []

        trace = langfuse_service.create_trace(
            name="MCP Validation",
            input={
                "resource_types": list(set(resource_types_found))[:8],
        ...
        try:
            ...
            # ── Check 1: Latest AWS provider version ──────────────────────────
            ...
            ok = True  # Non-blocking — don't fail the whole generation
