...
-  **Conversational Interface** - Chat with an AI assistant to define infrastructure requirements
-  **LLM-Powered Generation** - Structured requests to Terraform using OpenAI/Gemini
-  **Security-First** - Strict validation, no provisioners, least-privilege IAM
-  **Multi-Cloud** - AWS, GCP, Azure, and DigitalOcean support
-  **Async Workflows** - Background execution with real-time status updates
-  **Complete Terraform Lifecycle** - `init`, `plan`, `apply`, `destroy` automation
-  **10 IAM Role Types** - Predefined secure patterns (S3, SSM, CloudWatch, Secrets Manager, etc.)
-  **Workspace Isolation** - Each run in isolated directory with full state management
-  **Comprehensive Logging** - Full execution logs and outputs
- ¨ **Streamlit Frontend** - Beautiful, interactive UI for infrastructure management
 ...
 **[View detailed IAM role documentation â†’](docs/IAM_ROLES.md)**
...
│   │   ├── workflow_engine.py         # Async workflow orchestration
